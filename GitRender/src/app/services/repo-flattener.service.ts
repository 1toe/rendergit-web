...
export interface RenderDecision { include: boolean; reason: string; }
...
export class RepoFlattenerService {
  ...
  generateCxml(rendered: FileInfo[], repoUrl: string): string {
    ...
    rendered.forEach((f, idx) => {
      const safe = this.escapeXml(f.content || '');
  ...
  private escapeXml(t: string): string { return t.replace(/[<>&]/g, c => ({'<':'&lt;','>':'&gt;','&':'&amp;'}[c]!)); }
