...
export const useIconService = () => {
  const getIconName = useMemo(() => (iconKey: string): string => {
    const mapping = iconMappings.find(m => m.name === iconKey);
    return mapping ? mapping.lucideName : iconKey;
  }, []);

  const getIconDescription = useMemo(() => (iconKey: string): string => {
    const mapping = iconMappings.find(m => m.name === iconKey);
    return mapping ? mapping.description : '';
  }, []);

  const getAllIcons = useMemo(() => (): IconMapping[] => {
    return iconMappings;
  }, []);
